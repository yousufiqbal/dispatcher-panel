...
<div class="p-3 sm:p-6 max-w-2xl">
	<PageHeaderBack
		href="/admin/couriers/{data.courier.id}"
		backTitle="Back to {data.courier.name}"
		title="Assigned Stores"
		subtitle={data.courier.name}
	/>
	...
	<form method="POST" action="?/save" use:enhance>
		...
		<div class="flex gap-3">
			...
			<Button href="/admin/couriers/{data.courier.id}" variant="secondary">Cancel</Button>
		</div>
	</form>
</div>
